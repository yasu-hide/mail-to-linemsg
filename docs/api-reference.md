...
- まず extAddrId で対象アドレスを取得する
- 対象が存在しない場合は `ADDRESS_NOT_FOUND`（404）を返す
- ログイン済みユーザーに紐づくアドレス一覧と照合し、所有者確認を行う
- 所有者でない場合は `ADDRESS_NOT_OWNED`（404）を返す
- 所有者確認を通過した場合のみ addr_master から削除する
...
- 成功時は `{"msg":"Success","result":["{extAddrId}"]}` を返す
